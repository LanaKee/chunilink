...
const AchievementChip = ({ type, count = 0, total = 71 }: AchievementChipProps) => {
	...
	switch (chipInfo.type) {
		...
		case 'rainbow':
			headerStyle = {
				background:
					'linear-gradient(75deg, #DFFDFE 0%, #5472FB 20%, #5472FB 40%, #5472FB 60%, #FFFED8 75%, #FFFF73 87%, #DBFF58 100%)',
				fontWeight: 'bold',
				position: 'relative',
				overflow: 'hidden'
			}
	...
	return (
		<div className="w-full max-w-32 overflow-hidden rounded-md border border-gray-200 shadow-md">
			<div
				className={`flex w-full items-center justify-center px-2 py-1 ${textColorClass} relative`}
				style={headerStyle}
			>
				{chipInfo.type === 'platinum' && (
					<>
						...
						{/* 은은한 광택 효과 */}
						<div className="pointer-events-none absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-60" />
					</>
				)}
				{chipInfo.type === 'rainbow' && (
					<>
						...
						{/* 광택 효과 */}
						<div className="pointer-events-none absolute inset-0 z-10 bg-gradient-to-r from-transparent via-white/40 to-transparent opacity-60" />
					</>
				)}
				<span className="text-shadow-sm relative z-10">{chipInfo.text}</span>
			</div>
			<div className="flex w-full flex-col items-center justify-center bg-white py-3">
				<div className="text-4xl font-bold text-gray-800 md:text-5xl">{count}</div>
				<div className="text-lg font-normal text-gray-600 md:text-xl">/{total}</div>
			</div>
		</div>
	)
}
...
// 3가지 타입 예시 컴포넌트
export const ChunithmChipExamples = () => {
	return (
		<div className="rounded-xl border border-white/30 bg-white/30 p-6 shadow-xl backdrop-blur-md dark:border-white/10 dark:bg-black/20">
			...
			<div className="space-y-8">
				...
				{/* 금색 타입 */}
				<div>
					<h3 className="mb-3 text-xl font-semibold text-gray-800 dark:text-gray-200">금색 타입</h3>
					<div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
				...
				{/* 백금 타입 */}
				<div>
					<h3 className="mb-3 text-xl font-semibold text-gray-800 dark:text-gray-200">백금 타입</h3>
					<div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-5">
